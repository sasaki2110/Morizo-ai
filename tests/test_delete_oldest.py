#!/usr/bin/env python3
"""
FIFO原則による最古アイテム削除のテスト
inventory_delete_by_name_oldestツールの動作確認
"""
...
logger = logging.getLogger("test_delete_oldest")

class DeleteOldestTester:
    """FIFO原則による最古アイテム削除のテストクラス"""
    ...
    async def test_fifo_delete_oldest(self):
        """FIFO原則による最古アイテム削除のテスト"""
        logger.info("🧪 FIFO原則による最古アイテム削除テスト開始")
        ...
        try:
            ...
            if "削除" in response and ("完了" in response or "処分" in response):
                logger.info(f"✅ FIFO削除テスト成功: 最古アイテムの削除完了")
            else:
                logger.warning(f"⚠️ 不明な結果: {response[:100]}...")
            
        except Exception as e:
            logger.error(f"❌ FIFO削除テストエラー: {str(e)}")
        ...
        logger.info("🎉 FIFO原則による最古アイテム削除テスト完了")
...
async def main():
    """メイン関数"""
    tester = DeleteOldestTester()
    await tester.test_fifo_delete_oldest()
