...
**RAG検索（従来献立）側**: ❌ **古臭い手動選択**
- `_select_manual_alternative_dish`関数でハードコーディングされた代替料理選択
- AI制約解決エンジンが除外食材を正しく理解できていない
- 「もやしのナムル」等の固定選択肢を使用

#### 次の対応方針
1. **RAG検索の制約解決エンジン改善**: 除外食材を正しく処理するよう修正
2. **手動選択機能の削除**: AIネイティブな実装に統一
3. **LLM推論方式のRAG検索への適用**: 一貫したAIネイティブアプローチ
